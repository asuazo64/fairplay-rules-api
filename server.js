...
const CATEGORY_RULES = {
  // Etiquetas EXACTAS de los 9 botones del frontend.
  ...
  // Cobertura ampliada (ES/EN) para el resto de familias de reglas (1-25),
  ...
  "busqueda": ["7"], "search": ["7"],
  "mejorar": ["8"], "mejora": ["8"], "improve": ["8"], "improving": ["8"], "condiciones que afectan": ["8"],
  "linea de juego": ["8"], "area de swing": ["8"], "area de stance": ["8"], "doblar": ["8"], "romper rama": ["8"],
  "bola movida": ["9"], "bola en reposo": ["9"],
  "consejo": ["10"], "caddie": ["10"], "advice": ["10"], "alineacion": ["10"],
  "bola en movimiento": ["11"], "ball in motion": ["11"], "desviada": ["11"], "deflected": ["11"],
  "astabandera": ["13"], "bandera": ["13"], "flagstick": ["13"], "putting": ["13"],
  "dropear": ["14"], "drop": ["14"], "recolocar": ["14"], "procedimiento": ["14"], "lugar equivocado": ["14"], "wrong place": ["14"],
  "impedimento suelto": ["15"], "obstruccion movible": ["15"], "loose": ["15"], "movable": ["15"],
  "condicion anormal": ["16"], "obstruccion inamovible": ["16"], "agua temporal": ["16"], "terreno en reparacion": ["16"], "bola enterrada": ["16"], "abnormal": ["16"], "immovable": ["16"], "embedded": ["16"],
  "area de penalidad": ["17"], "penalizacion": ["17"], "penalidad": ["17"],
  "fuera de limites": ["18"], "provisional": ["18"], "golpe y distancia": ["18"], "out of bounds": ["18"], "stroke and distance": ["18"],
  "injugable": ["19"],
  "comite": ["20"], "arbitro": ["20"], "disputa": ["20"], "referee": ["20"],
  "stableford": ["21"], "maximo de golpes": ["21"], "otras formas": ["21"],
  "foursome": ["22"], "four-ball": ["23"], "fourball": ["23"],
  "equipos": ["24"], "team": ["24"],
  "discapacidad": ["25"], "disability": ["25"], "modificaciones": ["25"],
  "regla local": [],
};
...
// Extrae números de regla válidos (1-25) de un texto en cualquier idioma.
function extractRuleNumbers(text) {
  const out = [];
  if (!text) return out;
  const re = /\b([1-9]|1\d|2[0-5])\b/g;
  let m;
  while ((m = re.exec(String(text))) !== null) {
    const n = m[1];
    if (KB[n] && !out.includes(n)) out.push(n);
  }
  return out;
}
...
function buildKBContext(category, detectedRule) {
  ...
  //    FIX (caso 10 / rama doblada): también se busca dentro del texto de detectedRule,
  //    no solo en category — el propio nombre descriptivo que /phase1 genera para
  //    detectedRule (ej. "Mejora de la posición, línea de juego...") suele contener
  //    palabras clave de la regla correcta aunque el NÚMERO que /phase1 asignó esté mal
  //    (ej. citó "15.2a" en vez de "8.1a"). Además, ya no se detiene en el primer label
  //    que matchea: acumula TODOS los labels que coincidan, para que un número de regla
  //    mal detectado no excluya silenciosamente la regla realmente aplicable.
  let catKeys = [];
  const norm = normalizeCat(category);
  const normRule = normalizeCat(detectedRule);
  const combined = `${norm} ${normRule}`.trim();
  if (combined) {
    if (CATEGORY_RULES[norm]) {
      catKeys = catKeys.concat(CATEGORY_RULES[norm]);
    }
    const labels = Object.keys(CATEGORY_RULES).sort((a, b) => b.length - a.length);
    for (const label of labels) {
      if (label && combined.includes(label)) { catKeys = catKeys.concat(CATEGORY_RULES[label]); }
    }
  }
...
app.post("/phase1", async (req, res) => {
  try {
    ...
    // ── Detección temprana de preguntas de seguimiento (no son una situación nueva) ──
    // Causa raíz del Caso 4: mensajes como "cita la fuente exacta de esa Modelo F-6"
    // no describen una situación de golf y no encajan en el esquema JSON de facts,
    // por lo que el modelo produce texto libre y JSON.parse falla con un error genérico.
    // Se detecta el patrón aquí (antes de construir el prompt, para no desperdiciar
    // tokens) y se devuelve un error claro y accionable en vez de un crash silencioso.
    // Nota: una solución completa de UX (permitir un modo de "aclaración" separado de
    // "nueva situación") requiere cambios en el frontend (index.html), no solo aquí.
    // NOTA 2: se evita cualquier patrón que matchee solo por mencionar "regla local" + un
    // código (ej. "E-5") sin un verbo de solicitud explícito — este torneo usa reglas
    // locales reales (E-5, E-6) dentro de descripciones legítimas de situaciones, y ese
    // patrón más amplio producía falsos positivos bloqueando casos válidos.
    const followUpPatterns = /\b(cita|citame|dame|cual es|cuál es|pidele?|puedes citar)\b.{0,40}\b(fuente exacta|texto literal|fuente de esa)\b/i;
    if (followUpPatterns.test(description)) {
      addLog("phase1_followup_detected", { description: description.slice(0, 200) });
      return res.status(400).json({
        error: "Esta pregunta parece ser de seguimiento sobre un ruling o cita anterior, no una nueva situación de golf. Por ahora, este asistente solo procesa descripciones de situaciones nuevas. Describe la situación completa de nuevo, incluyendo tu pregunta.",
        followUp: true,
      });
    }
...
Extract facts and assumptions as JSON.`;
    ...
    let raw = await callClaude(system, userContent, 1024);
    let parsed;
    try {
      parsed = JSON.parse(cleanJSON(raw));
    } catch {
      // Un solo reintento explícito antes de rendirse — a veces el modelo agrega
      // texto extra o comentarios que rompen JSON.parse en el primer intento.
      try {
        const retryContent = `${userContent}\n\nIMPORTANT: Your previous response could not be parsed as valid JSON. Respond with ONLY the JSON object described above — no markdown, no code fences, no commentary before or after.`;
        raw = await callClaude(system, retryContent, 1024);
        parsed = JSON.parse(cleanJSON(raw));
      } catch {
...
9. Rule 8.1c check (mandatory whenever Rule 8 appears in the KNOWLEDGE BASE above): if the confirmed facts state that the player released, restored, or returned a bent/moved object to (approximately) its original position BEFORE making the stroke, and the object was not broken beyond restoration, you MUST apply Rule 8.1c and conclude there is NO PENALTY — do not default to the Rule 8.1a general penalty in that situation. Only withhold the no-penalty conclusion if the facts state the object was broken/could not be restored, or that restoration happened using a different object/material. CRITICAL: a confirmed fact is settled information, not a hypothesis — if a confirmed fact already states the object was released/restored before the stroke, treat that as PROVEN, not as "possibly happened" or "the player may have started restoring it." Do NOT hedge the conclusion, do NOT present it as conditional ("if the branch was fully restored..."), and do NOT invent extra requirements the actual rule text does not contain (e.g., Rule 8.1c does not require separately verifying that the original line of sight/view was restored — an unbroken object returning to its original position by its own elasticity after being released satisfies the rule on its own). State the final answer as a single definitive conclusion: NO PENALTY.
